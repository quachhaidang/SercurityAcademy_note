...
app.get('/api/public/lookup/:student_id', async (req, res) => {
    ...
    const grades = await db.all('SELECT subject, semester, academic_year, tx1, tx2, tx3, tx4, tx5, gk, ck, score, signature, hash FROM grades WHERE student_id = ?', [student_id]);
